...
data{
  ...
  int T_max; // Maximum number of trials

  int<lower=1, upper=P> participant_idx[N];
  int<lower=1, upper=E> electrode_idx[N];
  int<lower=1, upper=C> condition_idx[N];

  int<lower=1, upper=T_max> trials[N]; // y is a ragged array so this contains the enddings
  vector<lower=-pi(), upper=pi()>[T_max] y[N];
}
parameters{
...
model{
  ...
  // Observations
  for(i in 1:N){
    y[i, 1:trials[i]] ~ wrapped_cauchy(mean_angle[i], gamma[i]);
  }
}
