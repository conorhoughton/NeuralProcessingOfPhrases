...
cluster_perm <- function(df_lst, rank_thresh){
    ...
    test_stat <- abs(sapply(df_lst,FUN=function(x) mean(x$diff * sample(c(-1,1), replace=T, size=39))))
...
mean_res <- data %>%
                group_by(participant, electrode, condition, freq) %>%
                summarise(mean_phase=cabs(mean(phase))) %>%
                ungroup()# Across trials

mean_res$electrode <- electrode_info$electrode[mean_res$electrode]
mean_res <- mean_res %>% mutate(condition = paste(condition, freq, sep=" ")) %>% select(-c(freq))
...
for(i in 1:length(grp1_lst)){
  ...
  df$diff <- df[[grp1]] - df[[grp2]] # calculate condition difference
  df_lst  <- split(df, df$electrode)

  # main computation
  cluster_sizes  <- replicate(5000, cluster_perm(df_lst, r_val)) # generate the null
