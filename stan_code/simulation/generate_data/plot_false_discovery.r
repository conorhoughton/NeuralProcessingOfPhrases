...
files     <- list.files()
idxs      <- str_detect(string = files, pattern = "^FD_([a-z]+_)+([0-9]+_)+[0-9]+_")
fd_files  <- files[idxs]
...
result_df$order <- rep(c(3,5,7,1,4,6,8,2), 2)
...
ggsave(plot = p, filename ="false_discovery_rate.tiff", dpi=600, units = "in", height = 2.6, width = 5.2, compression="lzw")
